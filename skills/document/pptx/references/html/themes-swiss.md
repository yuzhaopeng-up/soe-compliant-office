...
4 套简约商务风的高反差配色。**每套都遵循"高级灰白底 + 单一高饱和高亮色"的极简原则**——这是简约商务风的灵魂,不允许混搭多个高亮色。
...
**适合**：工作汇报、方案路演、商业发布、AI/科技产品。最经典的简约商务风配色,绝不出错。
...
- ❌ **不允许混搭**（例如 IKB 蓝 + 柠檬黄同时出现作高亮）——彻底违反简约商务风"单一锚点色"原则
- ❌ **不允许用户自定义任意 hex 值**——委婉拒绝,展示 4 套预设让选
- ❌ **不要改灰阶变量**——`--paper` / `--grey-1/2/3` / `--ink` 跨主题统一,只换 accent
- ❌ **不要用渐变**——简约商务风拒绝任何渐变,所有色块必须纯色
- ❌ **不要给 accent 加阴影 / 圆角 / 透明度**——直角、纯色、不透明,这是简约商务风的硬规则
...
这套灰阶是经过校色的"高级灰",在任何 accent 色下都不抢戏。**不要**改成纯白（`#fff`）或纯黑（`#000`）——会损失简约商务风的"克制"质感。
