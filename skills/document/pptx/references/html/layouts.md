...
### B4. Grid 内容页间距上限（**重要,防止底行文字被裁切**）

当页面同时包含**标题区（kicker + h-xl + lead）+ 多行网格（grid-4/grid-6/grid-9 等）**时，标题区间距叠加容易压缩网格空间，导致底行卡片内容溢出被 `.frame` 的 `overflow:hidden` 裁切。

**间距上限规则**：
- `.frame` 的 `padding-top`：≤ `3vh`（不要用 `6vh`，太浪费纵向空间）
- `.h-xl` 的 `margin-bottom`：≤ `2vh`
- `.lead` 的 `margin-bottom` + `margin-top` 合计：≤ `3vh`
- 网格的 `margin-top`：≤ `2vh`

**推荐写法**（以含 grid-4 的页面为例）：
```html
<div class="frame" style="padding-top:2vh">
  <div class="kicker" data-anim>...</div>
  <h2 class="h-xl" style="margin-bottom:1vh" data-anim>...</h2>
  <p class="lead" style="margin-bottom:2vh; margin-top:.5vh" data-anim>...</p>
...
**自检方法**：生成后用浏览器 DevTools 审查 `.frame` 内最靠底部的文字元素，确认其 `getBoundingClientRect().bottom` ≤ frame 的 bottom。如果超出，说明内容溢出会被裁切。

### B5. 小屏适配（**重要，防止非 16:9 视口内容溢出**）

Windows 120% DPI 缩放、外接非 16:9 显示器等场景下，浏览器视口高度会小于标准 16:9 比例，导致纯 vw 单位的字号在短屏幕上放不下。template.html 的 CSS 已将所有大字号改为 `min(Xvw, Yvh)` 双约束（Y ≈ X × 1.78，即 16/9），确保标准 16:9 下 vw 始终生效，短屏下 vh 自动接管缩小字号。

**生成 slide 时的硬规则**：

1. **inline style 禁止纯 vw 字号**：所有 `font-size` 必须用 `min(Xvw, Yvh)` 格式，Y ≈ X × 1.78（向上取整）。例如 `font-size:min(7vw,12vh)` 而非 `font-size:7vw`
2. **hero 页 gap 用双约束**：`gap:min(Nvh,Mvw)`，M ≈ N × 1.78。例如 `gap:min(6vh,11vw)`
3. **不要用 min-height:80vh**：hero 页用 `align-content:center` 即可实现垂直居中，`min-height:80vh` 在短屏下会撑出更多空间反而让内容溢出
4. **网格 align-content 用 safe center**：`.grid-4` / `.grid-6` 等网格容器已在 CSS 中改为 `align-content:safe center`，当内容超出容器时自动 fallback 到 `start` 而非双向裁切
...
- 主题色从 `references/html/themes.md` 的 5 套预设里选一套,不允许自定义 hex 值
...
**图片槽位**：无（WebGL 流体背景即主视觉，封面放图是可选项而非必选项）
...
<section class="slide hero dark">
  ...
  <div class="frame" style="display:grid; gap:min(4vh,7vw); align-content:center">
...
- 用 `align-content:center` + gap 的 `min(Nvh,Mvw)` 双约束让内容垂直居中且小屏不溢出
...
<section class="slide hero light">
  ...
  <div class="frame" style="display:grid; gap:min(6vh,11vw); align-content:center">
    <div class="kicker" data-anim>Act I</div>
    <h1 class="h-hero" style="font-size:min(8.5vw,15vh)" data-anim>硬数据</h1>
...
**图片槽位**：1 个（`<figure class="frame-img r-16x10|r-4x3">` 右列图位）
...
**图片槽位**：2-6 个（`<figure class="frame-img" style="height:NNvh">` 网格图位）
...
<section class="slide hero dark">
  ...
  <div class="frame" style="display:grid; gap:min(8vh,14vw); align-content:center">
    <div class="kicker" data-anim>The Question</div>
    <h1 class="h-hero" style="font-size:min(7vw,12vh); line-height:1.15">
...
<section class="slide light" data-animate="quote">
  ...
  <div class="frame" style="display:grid; gap:min(5vh,9vw); align-content:center">
    <div class="kicker" data-anim>Quote · 金句</div>
    <blockquote style="font-family:var(--serif-zh); font-weight:700; font-size:min(5.8vw,10vh); line-height:1.2; letter-spacing:-.01em; max-width:72vw">
...
**图片槽位**：1 个（`<figure class="frame-img r-3x4|r-1x1">` 右列辅助图位）
