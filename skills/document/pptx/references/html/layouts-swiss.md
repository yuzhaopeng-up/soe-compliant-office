...
> ⚠️ 这套版式与风格 A（典雅画报风）**不通用**。类名同名但语义不同(例如 `h-hero` 在风格 A 是衬线,在风格 B 是无衬线极细 200)。一份 deck 只能选一套。
...
`assets/template-swiss.html` 与 `references/html/swiss-layout-lock.md` 中登记的 S01-S22 版式

生成正文页时不要把简约商务风当成“自由组合的风格包”。默认只能使用 `references/html/swiss-layout-lock.md` 登记的 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。
...
- 大标题遵循原始 PPT 的实际页面用法:主标题 `font-weight:200`,重点词/数字 `font-weight:300`;不要因为旧 CSS helper 里残留过 800/900 就把简约商务风大标题加粗
...
**5. 小屏适配(已内置 CSS 双约束 + @media 兜底)**
template-swiss.html 已内置两层小屏防护:
- **CSS 类级**:所有大字号已改为 `min(Xvw, Yvh)` 双约束(Y ≈ X × 1.78),标准 16:9 下 `vw` 始终生效,短屏下 `vh` 自动接管缩小
- **@media(max-height:800px)**:chrome/foot 字号 11px→10px,stat-note 下限 12px→11px,回收约 8px 纵向空间
- **grid `align-content:safe center`**:grid-4/grid-6 等网格容器当内容超出时自动 fallback 到 `start`,防止双向裁切
- **inline style 严禁纯 vw 字号**:所有 `style="font-size:Xvw"` 必须改为 `style="font-size:min(Xvw, Yvh)"`,Y ≈ X × 1.78

**6. 底部分页安全区:主内容最低处不要触及 nav**
...
- 简约商务风图片优先比例:S22 顶部横幅 `21:9`;S15/S16 多图格统一 `21:9` 或统一 `16:10`
...
- 所有图片素材必须尽量贴合单一 accent 色、Helvetica/Inter 气质、12/16 列网格、直角纯色、无渐变/阴影/圆角
- 生成图只保留核心图像本身,不要把页眉、页脚、标题、页码、角标、边框、署名画进图片里

**版式多样性硬规则**
简约商务风主题有 22 个登记版式,生成时要主动展示版式系统,不要把所有内容都做成 `head + grid-reveal + card`:
...
不要只看 HTML/CSS。简约商务风模板的还原度要同时从**浏览器视觉**和**代码结构**判断:

1. 同时打开三份页面:原始参考 PPT、当前 `template-swiss.html` 或生成页、正在修改的测试 PPT。原始参考路径是 `assets/template-swiss.html` 与 `references/html/swiss-layout-lock.md` 中登记的 S01-S22 版式。
...
**图片槽位**：可改造（原始矩阵卡可替换为 `.frame-img.r-21x9` / `.r-16x10` 图片格）
...
**图片槽位**：可改造（微卡可替换为 `.frame-img.r-21x9` / `.r-16x10` 图片格）
...
**图片槽位**：1 个（`.hero-img-wrap` 60vh 顶部主图，21:9）
...
**注意**:
- 图片优先用 `images/{页号}-{语义}.png` 本地文件(用户提供素材或后续补齐的本地素材),不要默认外链 unsplash
...
### P24 · 多图证据墙（`Swiss Evidence Grid`，实验，默认禁用）

**用途**:三张同类型图片/截图/图表并列,展示证据链或多案例对比。
**适用内容类型**:**2-3 张同类图片**。适合 UI 截图重绘、流程图三段、三个案例实拍、三张数据小图。不同类型混放会破坏简约商务风秩序。
**图片槽位**：2-3 个（`.frame-img.h-22|h-26` 图位组）
...
- 如果用户原始截图比例混乱,先与用户确认是否提供统一比例素材；没有素材时使用固定槽位占位，不调用 AI 生图
...
| 单图解释论点 / 图文混排 | P23 图文分栏 |
| 2-3 张图片/截图/图表证据链 | P24 多图证据墙 |
...
| 1 张核心图片 + 一段解释 | P23 图文分栏 | P22(除非图片是主角且有 KPI) |
