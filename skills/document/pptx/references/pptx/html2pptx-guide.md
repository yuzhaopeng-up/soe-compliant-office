...
- ✅ **中文字体（推荐用于中文内容）**：`"Microsoft YaHei"` (微软雅黑), `"Source Han Sans CN"` (思源黑体), `"Source Han Serif CN"` (思源宋体), `"Alibaba PuHuiTi"` (阿里巴巴普惠体), `"STHeiti"` (华文细黑)
- ✅ **Web-safe fonts（英文/通用）**：`Arial`, `Helvetica`, `Times New Roman`, `Georgia`, `Courier New`, `Verdana`, `Tahoma`, `Trebuchet MS`, `Impact`, `Comic Sans MS`
- ❌ Wrong: `'Segoe UI'`, `'SF Pro'`, `'Roboto'`, custom fonts - **Might cause rendering issues**

**字体使用建议**：
- 中文内容优先使用 `"Microsoft YaHei", "微软雅黑"`
- 需要打印时使用 `"Source Han Serif CN", "思源宋体"`（宋体）
- 英文内容使用 `Arial` 或 `Helvetica`
- 可以使用字体栈作为降级方案：`font-family: "Microsoft YaHei", "微软雅黑", Arial, sans-serif;`
...
**If Playwright fails** (e.g. "Executable doesn't exist", "Browser not found", Chromium version errors): run `npx playwright install` or `npx playwright install chromium`, then retry. Do not run this proactively at startup—only when html2pptx has already failed with a browser-related error.
