...
def parse_use_element(use_match: str) -> dict[str, str | float]:
    ...
    # Extract optional stroke-width override for any future stroke-style icon library.
...
def generate_icon_group(attrs: dict[str, str | float], elements: list[str], style: str, base_size: float) -> str:
    ...
    if style == 'stroke':
        # Default to 2 so future stroke-style libraries render consistently when
        # callers omit an explicit stroke-width.
