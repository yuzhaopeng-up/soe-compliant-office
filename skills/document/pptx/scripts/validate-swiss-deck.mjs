...
slides.forEach((slide) => {
  ...
  if (!layout) {
    errors.push(`Slide ${slide.idx}: missing data-layout. 简约商务风版式锁定要求 S01-S22 或 SWISS-COVER-ASCII/SWISS-CLOSING-ASCII。`);
  ...
  if (!isStatement && /text-align\s*:\s*center/i.test(topChunk)) {
    errors.push(`Slide ${slide.idx}: top title area contains text-align:center. 简约商务风正文标题应左对齐。`);
  }
...
if (errors.length) {
  console.error('简约商务风 deck 校验失败:');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log(`简约商务风 deck 校验通过: ${slides.length} 页。`);
