...
**CRITICAL TOC CAVEAT**: `TableOfContents` creates a TOC field placeholder. In many environments, opening the file will not auto-render TOC entries. If you need a deterministic result without Win COM, run:

`python scripts/render_toc_static.py <file.docx>`

This converts TOC to static clickable entries from headings. Page numbers are intentionally not rendered.
