...
Usage:
    ...
    python3 palette.py --title "报告" --type report --content content.json --out tokens.json
...
Body fonts use ReportLab system fonts; CJK content auto-selects a CJK-capable font.
...
def _extract_text_from_content(content_path: str) -> str:
    """
    Read content.json and flatten all text fields into a single string
    so that CJK detection can be performed without a full render.

    Silently returns an empty string on any read/parse error so that
    a missing or malformed file never blocks token generation.
    """
    try:
...
def build_tokens(
    ...
    """
    Build the full design token dict from document metadata.

    Args:
        title:             Document title.
        doc_type:          Document type key (must be a key in PALETTES).
        author:            Author name (optional).
        date:              Date string (optional).
        accent_override:   Override accent colour as hex string (optional).
        cover_bg_override: Override cover background colour as hex (optional).
        content_text:      Flattened body text sample used for CJK detection.
                           Pass the result of _extract_text_from_content() here.

    Returns:
        Token dict consumed by pdf_cover.py, pdf_render_body.py, etc.
    """
    palette   = PALETTES.get(doc_type, PALETTES["general"]).copy()
    mood      = palette["mood"]
    font_pair = FONT_PAIRS.get(mood, SYSTEM_FALLBACK)

    # Detect CJK in title, author, or body text sample.
    # The "cjk" flag is consumed by pdf_render_body.py so it can register
    # a CJK-capable font even when the content scan hasn't run yet.
    is_cjk = _has_cjk(title + " " + author + " " + content_text)
    ...
    tokens = {
        ...
        "date":     date,
        "doc_type": doc_type,
        # CJK flag — render_body.py reads this to decide whether to register
        # a CJK-capable font before building ParagraphStyles.
        "cjk":      is_cjk,
        ...
        # Monospace font used by ``code`` / ``code_lang`` / ``math_fallback`` styles.
        # Defaults to "Courier" so non-CJK documents keep their authentic
        # monospace look.  When pdf_render_body.py detects CJK content it patches
        # this key to the CJK font alias so Chinese / Japanese comments render
        # instead of missing-glyph boxes; the (acceptable) trade-off is that
        # mixed CJK code blocks lose strict monospace alignment.
        "font_mono_rl":     "Courier",
...
def main():
    ...
    parser.add_argument("--content", default="",
                        help="Path to content.json used for CJK language detection. "
                             "When the body text contains CJK characters the generated "
                             "tokens will include cjk=true so render_body.py can "
    ...
    content_text = _extract_text_from_content(args.content) if args.content else ""
    tokens = build_tokens(
